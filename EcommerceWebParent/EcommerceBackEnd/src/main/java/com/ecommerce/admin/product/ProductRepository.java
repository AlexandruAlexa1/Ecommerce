...
public interface ProductRepository extends PagingAndSortingRepository<Product, Integer> {
	...
	@Query("UPDATE Product p SET p.questionCount = (SELECT COUNT(q.id) FROM Question q WHERE q.product.id = ?1 AND q.approvalStatus = true) WHERE p.id = ?1")
	@Modifying
	public void updateQuestionCount(Integer productId);
	
	@Query("UPDATE Product p SET p.answerCount = (SELECT COUNT(q.id) FROM Question q WHERE q.product.id = ?1 AND q.answerContent != '') WHERE p.id = ?1")
	@Modifying
	public void updateAnswerCount(Integer productId);
}
