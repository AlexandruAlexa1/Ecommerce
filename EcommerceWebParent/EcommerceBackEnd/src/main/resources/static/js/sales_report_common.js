...
function setupButtonEventHandlers(reportType, callbackFunction) {
	$(".button-sales-by" + reportType).on("click", function() {
		...
		if (period) {
			$("#divCustomDateRange" + reportType).css("display", "none");
			callbackFunction(period);
		} else {
			$("#divCustomDateRange" + reportType).css("display", "block");
		}
	});
...
function setSalesAmount(period, reportType, labelTotalItems) {
	...
	$("#textAvgGrossSales" + reportType).text(formatCurrency(totalGrossSales));
	$("#textAvgNetSales" + reportType).text(formatCurrency(totalNetSales));
